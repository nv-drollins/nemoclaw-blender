...
ssh -F "$SSH_CONFIG" "openshell-$SANDBOX" bash /tmp/repair-sandbox-mcporter-wrapper.sh "$SPARK_IP"

ssh -F "$SSH_CONFIG" "openshell-$SANDBOX" /sandbox/bin/mcporter --help >/dev/null
echo "mcporter installed in sandbox $SANDBOX"
